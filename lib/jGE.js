...
class jGE extends ShowObj{
    ...
    Init(cfg) {
        ...
        this.__bind_helper(setting.dom,["mousemove","mousedown","mouseup","click","dblclick","touchstart","touchmove","touchend"],(e)=>{this.keyboards.forEach(kb=>{ kb.pointHandle(e,GetEventPosition(e));})});
    ...
    //判断是否某点在某路径内
    IsInIt(point,path){
        path.render(this.run.context2D);
        return this.run.context2D.isPointInPath(point.x,point.y);
    };
    ...
    constructor(){
        super();
        this.version = [3,1,1,"β"];//大版本不兼容，中版本加功能，小版本修bug
        this.setting = {};
