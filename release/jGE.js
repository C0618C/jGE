...
*/
class ObjectFactory extends Manager{
    constructor(_jGE){
        super(_jGE,"对象工厂");
        ...
        _jGE.one("jGE.Config.Loaded.object",objCfg=>{
        ...
        _jGE.one("jGE.Resource.Loaded",c=>{
            this.objectCfg.forEach(t=>t.items.forEach(i=>{
                if(i.type =="video") i.video = _jGE.get(i.res);
                else if(/^(?:sprite|anima)$/.test(i.type)) i.img = _jGE.get(i.res);
            }));
        })
        ...
        c.items.forEach(i=>{
            ...
            for(let n in i){
                ...
                switch(n){          //TODO:完成其它配置文件预处理并进行测试！！！
                    ...
                    //         if(RegExp.$1 == "video") tSetting.video = i.res;
                    //         else tSetting.img = i.res;
                    //     }
                    //     break;
                    default:
                        tSetting[n]=i[n];
